...
import type { LoginExpressRequest, RegisterExpressRequest } from "./types/customAuthRequest";
...
export class AuthRouteHandler {
  ...
  async register(
    ...
    try {
      ...
      return res.status(registerResponseData.status).json(registerResponseData.body);
    } catch (error) {
      next(error);
    }
  }
